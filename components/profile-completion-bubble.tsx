  ...
  return (
    <div className="fixed bottom-6 left-6 z-40 w-80 max-w-[calc(100vw-3rem)] bg-white rounded-2xl border border-gray-100 shadow-xl p-5">
      <div className="flex items-start justify-between mb-1">
        <h3 className="text-sm font-semibold text-foreground">{t.profileCompletion.title}</h3>
        <button
          onClick={() => setAndPersist(true)}
          aria-label={t.profileCompletion.minimize}
      ...
      <div className="mb-1 flex items-center justify-between">
        <span className="text-xs font-medium text-muted-foreground">{t.profileCompletion.progress}</span>
      ...
      >
        {ctaLabel || t.profileCompletion.title} <ArrowRight className="w-4 h-4" />
      </Link>
    </div>
  );
}
