  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEs ? 'Firma de Contrato' : 'Contract Signing'}</DialogTitle>
          <DialogDescription>
        ...
        {mode === 'submitting' ? (
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <p className="text-sm text-muted-foreground">{isEs ? 'Enviando tu firma...' : 'Submitting your signature...'}</p>
          </div>
        ) : mode === 'sign' ? (
          <div className="space-y-4">
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
              <strong>{isEs ? 'Sube la imagen de tu firma' : 'Upload your signature image'}</strong>. {isEs ? 'El fondo blanco se eliminará automáticamente.' : 'The white background will be removed automatically.'}
            </div>
            <input
              ref={fileInputRef}
              type="file"
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <Button variant="outline" size="sm" onClick={clearSignature}>
                    <X className="w-3.5 h-3.5 mr-1" /> {isEs ? 'Quitar' : 'Remove'}
                  </Button>
                  <div className="flex gap-2">
                    <Button variant="outline" onClick={() => setMode('view')}>{isEs ? 'Cancelar' : 'Cancel'}</Button>
                    <Button onClick={handleSubmit} className="gap-2">
                      <Upload className="w-4 h-4" /> {isEs ? 'Confirmar Firma' : 'Confirm Signature'}
                    </Button>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-4">
            {process?.contract_url ? (
              ...
              <div className="bg-gray-50 rounded-xl p-12 text-center text-muted-foreground">
                {isEs ? 'PDF del contrato aún no disponible.' : 'Contract PDF not available yet.'}
              </div>
            )}
            {process?.contract_status === 'pending' && (
              <div className="flex justify-end">
                <Button onClick={() => setMode('sign')} className="gap-2">
                  <Pen className="w-4 h-4" /> {isEs ? 'Firmar' : 'Sign'}
                </Button>
              </div>
            )}
          </div>
