...
export default function ApplicantTimesheetPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({
        title: isEs ? 'Horas enviadas' : 'Hours submitted',
        description: isEs ? 'Tu empleador las revisará y aprobará.' : 'Your employer will review and approve them.',
      });
    } catch (err) {
      toast({
        title: isEs ? 'No se pudieron enviar las horas' : 'Could not submit hours',
        description: err instanceof Error ? err.message : (isEs ? 'Intenta de nuevo.' : 'Try again.'),
        variant: 'destructive',
      });
    } finally {
      setSubmitting(false);
  ...
  if (hiredProcesses.length === 0) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-foreground">{t.applicant.timesheetTitle}</h2>
          <p className="text-muted-foreground mt-1">{isEs ? 'Reporta tus horas mensuales una vez que seas contratado para un rol.' : 'Report your monthly hours once you are hired for a role.'}</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-8 text-center">
          <p className="text-sm text-muted-foreground">
            {isEs ? 'No tienes un contrato activo aún. Esta sección se desbloqueará una vez que una empresa te contrate.' : 'You do not have an active contract yet. This section unlocks once an employer hires you.'}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-foreground">{t.applicant.timesheetTitle}</h2>
          <p className="text-muted-foreground mt-1">{isEs ? 'Reporta las horas trabajadas este mes para cada contrato.' : 'Report the hours you worked this month for each contract.'}</p>
        </div>
      ...
      {existingTimesheet?.status === 'submitted' && (
        <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-4 py-3">
          ...
          {isEs ? 'Enviado — esperando aprobación del empleador.' : 'Submitted — waiting for your employer to approve.'}
        </div>
      )}
      {existingTimesheet?.status === 'approved' && (
        <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg px-4 py-3">
          ...
          {isEs ? 'Aprobado. ' : 'Approved. '}{existingTimesheet.invoice_url && (
            <a href={existingTimesheet.invoice_url} target="_blank" rel="noreferrer" className="underline font-medium">
              {isEs ? 'Ver extracto de facturación' : 'View billing statement'}
            </a>
          )}
        </div>
      )}
      {existingTimesheet?.status === 'rejected' && (
        <div className="flex items-start gap-2 text-sm text-red-700 bg-red-50 border border-red-100 rounded-lg px-4 py-3">
          ...
          <span>{isEs ? `Devuelto para correcciones${lastRejection?.comment ? `: ${lastRejection.comment}` : '.'} Ajusta las horas y reenvía.` : `Sent back for corrections${lastRejection?.comment ? `: ${lastRejection.comment}` : '.'} Adjust the hours below and resubmit.`}</span>
        </div>
      )}
      ...
      {hasEnded && (
        <div className="flex items-center gap-2 text-sm text-blue-700 bg-blue-50 border border-blue-100 rounded-lg px-4 py-3">
          ...
          {isEs
            ? `Este contrato finalizó el ${formatDate(selectedProcess!.contract_end_date! + 'T00:00:00', { month: 'short', day: 'numeric', year: 'numeric' })}. Aún puedes enviar horas hasta ${formatDate(endMonth + '-01', { month: 'long', year: 'numeric' })}.`
            : `This contract ended on ${formatDate(selectedProcess!.contract_end_date! + 'T00:00:00', { month: 'short', day: 'numeric', year: 'numeric' })}. You can still submit hours through ${formatDate(endMonth + '-01', { month: 'long', year: 'numeric' })}.`}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-1">
            ...
            <h3 className="font-semibold text-foreground w-40 text-center capitalize">
          ...
          <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
            ...
            {isEs ? 'Total: ' : 'Total: '}<span className="font-semibold text-foreground">{total.toFixed(2)}h</span>
          </div>
        </div>
        ...
        <div className="space-y-3 min-w-[560px]">
          {groupDaysByWeek(days).map(({ week, days: weekDays }) => (
            <div key={week} className="grid grid-cols-[80px_repeat(5,1fr)] gap-2 items-center">
              <span className="text-xs text-muted-foreground">{isEs ? `Semana ${week}` : `Week ${week}`}</span>
              {weekDays.map((d) => {
                const isZero = d.hours === 0;
                return (
                  <div key={d.date} className="flex flex-col items-center gap-1">
                    <span className="text-xs text-muted-foreground capitalize">
                      {formatDate(d.date + 'T00:00:00', { weekday: 'short', day: 'numeric' })}
                    </span>
                    <div
        ...
        {!isLocked && (
          <div className="mt-6 flex justify-end">
            <Button onClick={handleSubmit} disabled={submitting} className="gap-2">
              ...
              {submitting ? (isEs ? 'Enviando...' : 'Submitting...') : (isEs ? 'Enviar horas' : 'Submit hours')}
            </Button>
          </div>
        )}
      </div>
