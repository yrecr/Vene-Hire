...
export default function EmployerDashboard() {
  ...
  const activeProcesses = getProcessesForEmployer(employerId).filter(
    (p) => p.status === 'active'
  );

  const pendingInterviews = getInterviewsForEmployer(employerId).filter(
    (i) => i.status === 'pending'
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        <StatCard icon={Star} label="Shortlisted" value={3} />
      </div>
